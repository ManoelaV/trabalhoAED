...
int main() {
    ...
    // Array para armazenar os índices dos Pokémon selecionados
    int indices[50];
    for (int i = 0; i < 50; i++) {
        ...
        indices[i] = indice_aleatorio;
    }

    // Imprime as cartas dos Pokémon selecionados
    for (int i = 0; i < 50; i++) {
        printf("Imprimindo carta do Pokemon %d\n", indices[i] + 1);
        imprime_carta(&pokedex[indices[i]]);
    }
